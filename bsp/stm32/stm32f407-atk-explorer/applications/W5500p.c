...
{
	HAL_GPIO_WritePin(W5500_CS_GPIO_Port, W5500_CS_Pin, GPIO_PIN_RESET);;//置W5500的SCS为低电平
	...
	HAL_GPIO_WritePin(W5500_CS_GPIO_Port, W5500_CS_Pin, GPIO_PIN_SET);; //置W5500的SCS为高电平
}
...
unsigned char Read_W5500_1Byte(unsigned short reg)
{
  ...
  uint8_t data=0;
	...
	i=HAL_SPI_Receive(&hspi1,&data, 1 ,200);
	if(i!=HAL_OK){
		rt_kprintf("SPI read 1err:%d",i);
	}
	SPI1_Send_Byte(0x00);//发送一个哑数据
	//i=SPI_I2S_ReceiveData(SPI1);//读取1个字节数据
	i=HAL_SPI_Receive(&hspi1,&data, 1 ,200);
	if(i!=HAL_OK){
		rt_kprintf("SPI read 2err:%d",i);
	}
	HAL_GPIO_WritePin(W5500_CS_GPIO_Port, W5500_CS_Pin, GPIO_PIN_SET);;//置W5500的SCS为高电平
...
unsigned char Read_W5500_SOCK_1Byte(SOCKET s, unsigned short reg)
{
	...
	i=HAL_SPI_Receive(&hspi1,&data, 1 ,2);
	if(i!=HAL_OK){
		rt_kprintf("SPI read 1byte err:%d",i);
	}//i=SPI_I2S_ReceiveData(SPI1);
	SPI1_Send_Byte(0x00);//发送一个哑数据
	i=HAL_SPI_Receive(&hspi1,&data, 1 ,2);
	if(i!=HAL_OK){
		rt_kprintf("SPI read 2byte err:%d",i);
	}//i=SPI_I2S_ReceiveData(SPI1);//读取1个字节数据
...
unsigned short Read_W5500_SOCK_2Byte(SOCKET s, unsigned short reg)
{
  ...
  unsigned short data;
	...
	HAL_SPI_Receive(&hspi1,&i, 1 ,1);//i=SPI_I2S_ReceiveData(SPI1);
	SPI1_Send_Byte(0x00);//发送一个哑数据
	HAL_SPI_Receive(&hspi1,&i, 1 ,1);//i=SPI_I2S_ReceiveData(SPI1);//读取高位数据
	SPI1_Send_Byte(0x00);//发送一个哑数据
	data=i*256;
	//i*=256;
	HAL_SPI_Receive(&hspi1,&i, 1 ,1);//i+=SPI_I2S_ReceiveData(SPI1);//读取低位数据
	data+=i;
...
unsigned short Read_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr)
{
	...
	else//如果最大地址超过W5500接收缓冲区寄存器的最大地址
	{
		...
		SPI1_Send_Byte(VDM|RWB_READ|(s*0x20+0x18));//写控制字节,N个字节数据长度,读数据,选择端口s的寄存器
		HAL_SPI_Receive(&hspi1,&j, 1 ,1);//HAL_SPI_Receive(&hspi1,&j, 1 ,1);//j=SPI_I2S_ReceiveData(SPI1);
...
{
	HAL_GPIO_WritePin(W5500_RST_GPIO_Port, W5500_RST_Pin, GPIO_PIN_RESET);//GPIO_ResetBits(W5500_RST_GPIO_Port, W5500_RST_Pin);//复位引脚拉低
	rt_thread_mdelay(2);
	HAL_GPIO_WritePin(W5500_RST_GPIO_Port, W5500_RST_Pin, GPIO_PIN_SET);//GPIO_SetBits(W5500_RST_GPIO_Port, W5500_RST_Pin);//复位引脚拉高
...
void W5500_Init(void)
{
	...
	rt_thread_mdelay(10);//延时10ms,自己定义该函数
