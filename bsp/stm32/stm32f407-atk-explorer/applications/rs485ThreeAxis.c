...
//<<Ñ¹²îÊ½³Á½µÒÇ GY-STC-2000>> Ä¬ÈÏ²¨ÌØÂÊ9600  modbusµØÖ· 0xb1(ÓĞÎó) ¼û»úÉí±êÇ©ºó2Î»
/*
...
#define  MSGPOOL_LEN   200 //485Êı¾İ×î´óÁ¿  ´óÓÚ1kĞèÒªĞŞ¸Ä´Ë´¦
//¶ÓÁĞµÄ¶¨Òå
...
rt_err_t threeAxisUartRec(uint8_t dat)
{
		...
		if(recFlag==RT_TRUE){
				return rt_mq_send(&threeAxismque, &dat, 1);  //ÊÕµ½Êı¾İºó¾ÍÍù¶ÓÁĞÀï¶ª
		}
		else
			  return RT_FALSE;
}
...
//
//´´½¨»·Á÷ÓÃµ½µÄ»¥³âÁ¿ºÍÏûÏ¢¶ÓÁĞ  mainÖĞµ÷ÓÃ
...
//	  uint16_t ret =0;
	  ...
	  rt_kprintf("%sthreeAxis send:",sign);
		...
		rt_kprintf("\n");
    len=0;
		if(rt_mq_recv(&threeAxismque, buf+len, 1, 3000) == RT_EOK){//µÚÒ»´Î½ÓÊÕÊ±¼ä·Å³¤µã  ÏàÓ¦Ê±¼äÓĞ¿ÉÄÜ±È½Ï¾Ã
				len++;
		}
		while(rt_mq_recv(&threeAxismque, buf+len, 1, 10) == RT_EOK){//115200 ²¨ÌØÂÊ1ms 10¸öÊı¾İ
				len++;
		}
		if(len!=0){
