...
void readPdFreqDischarge()
{
	  ...
	  rt_kprintf("%sPdFreqDiach send:",sign);
		...
		memset(buf,0,LENTH);
    len=0;
		if(rt_mq_recv(modDev[chanl.partDischag].uartMessque, buf+len, 1, 3000) == RT_EOK){//µÚÒ»´Î½ÓÊÕÊ±¼ä·Å³¤µã  ÏàÓ¦Ê±¼äÓĞ¿ÉÄÜ±È½Ï¾Ã
				len++;
		}
		while(rt_mq_recv(modDev[chanl.partDischag].uartMessque, buf+len, 1, 10) == RT_EOK){//115200 ²¨ÌØÂÊ1ms 10¸öÊı¾İ
				len++;
		}
		if(len!=0){
