#include "board.h"
//#include "stmflash.h"
//<<—π≤Ó Ω≥¡Ωµ“« GY-STC-2000>> ƒ¨»œ≤®Ãÿ¬ 9600  modbusµÿ÷∑ 0xb1(”–ŒÛ) º˚ª˙…Ì±Í«©∫Û2Œª   –≠“ÈŒƒµµ”–ŒÛ
/*
...
// ∑¢                   01 04 00 01 00 04 A0 09 
//[∑¿Õ‚∆∆]threeAxis send:9  4  0  1  0  4 a1 41 
//  ’         01 04 08 0B CA FE 8D 00 03 03 80 C7 23 
//[∑¿Õ‚∆∆]rec: 9  4  6  b 73  0 9a  0 17 fc 32 69 b4
typedef struct{
		float temp;
...
void readThreeTempAcc(int num)
{
    ...
    len=0;
		memset(buf,0,LENTH);
		while(rt_mq_recv(uartDev[sheet.threeAxiss[num].useUartNum].uartMessque, buf+len, 1, 500) == RT_EOK){//115200 ≤®Ãÿ¬ 1ms 10∏ˆ ˝æ›
				len++;
		}
		if(len!=0){
...
static uint16_t threeAxisJsonPack()
{
	  ...
	  cJSON_AddStringToObject(root, "acuId",(char *)packFlash.acuId);
		...
		sprinBuf=rt_malloc(20);//20∏ˆ◊÷∑˚¥Æ≥§∂» πª”√¡À
...
{					
		...
		for(int i=0;i<THREEAXIS_485_NUM;i++){
			...
			if(workFlag==RT_TRUE){
					rt_kprintf("%s¥Ú∞¸≤…ºØµƒTHREEAXIS ˝æ›\r\n",sign);
